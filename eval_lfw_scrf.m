function [acc, evaltime] = eval_lfw_scrf(w_scrf, datanames, datanums, sds, esds, verbose)
startup_directory;
olddim = 250;
nlabel = size(w_scrf.nodeWeights,2);
addpath('model/scrf');
...
for i = 1:length(datanums),
    ...
    [proj_block, proj_sp] = create_mapping(sp,num_sp,w_scrf.params.dim,olddim);
    
    num_sp = X.numNodes;
    X.mapping_block = proj_block;
    X.mapping_sp = proj_sp';
    
    
    tS = tic;
    labelprob = inference_scrf(w_scrf, X, nlabel, 0);
    tE = toc(tS);
